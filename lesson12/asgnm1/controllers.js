...
module.exports.display1 = (req, res) => {
    ...
    Game.findById(gameId).then(games).catch(err);
...
module.exports.displayall = (req, res) => {
    ...
    if (count > maxCount) {
        ...
        Game.find().skip(offset).limit(maxCount).then(games).catch(err);
...
module.exports.gamesDeleteOne = function(req, res) {
    ...
    Game.findByIdAndRemove(gameId).then(deletedGame).catch(err);
...
module.exports.updatepatch = (req, res) => {
    const gameId = req.params.gameId;
    Game.findById(gameId).select("-reviews -publisher").then(game).catch(err);
    const response = {
        status: 204,
        message: game
    };
